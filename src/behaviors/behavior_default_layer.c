...
static int set_default_layer(uint8_t layer) {
    // TODO: implement
}
...
static int apply_default_layer_config(struct zmk_endpoint_instance endpoint) {
    ...
    int ret = set_default_layer(layer);
    if (ret < 0) {
        LOG_WRN(
            "Could not apply default layer from settings. Perhaps something in "
            "the code/keymap "
...
static int save_default_layer_setting(uint8_t layer,
    ...
    int ret = k_work_reschedule(&df_layers_save_work,
